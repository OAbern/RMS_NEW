 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '出版物名称', name: 'publicationName', align: 'left', minWidth: 200 },
                { display: '所在单位', name: 'unitAuthor', width: 100 },
                { display: '类别', name: 'publicationType', width: 100 },
                { display: '出版单位', name: 'publisher' , width: 150},
                { display: '书号（ISBN）', name: 'iSBN' , width: 150},
                { display: '奖励等级', name: 'awardingGrades' , width: 100},
                { display: '出版年月', name: 'publishedTime' , width: 100},
                { display: '总奖励金额（万元）', name: 'totalPrize' , width: 150},
                { display: '扣减特聘岗位', name: 'deductionsDistPosts' , width: 150},
                { display: '实际获得奖励金额（万元）', name: 'actualAward' , width: 200},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewScienceBookDetail.action?flag=modify&infoid='+row.bookId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.publicationName);
                                }else
                                {
                                	checkedIds.push(this.bookId);
                                	checkedNames.push(this.publicationName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteScienceBook.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
