                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteTeacherAwardsRecord.action?recordIdString='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
                    ...
                    case "Excel":
                        ...
                        else
                        {
                        	...
                        	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                        	{
                        		if(result)
                        		{
                        			link = 'downLoadExcelbyFactor/downloadDynamicDataRecordExcel.action?factorName=id&factorValue='+checkedIds+'&classNum=3';
                        			downloadExcel(link);
                        		}
                        	});
                        }
