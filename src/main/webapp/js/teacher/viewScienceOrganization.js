 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '机构名称', name: 'organizationName' , align: 'left', minWidth: 200 },
                { display: '机构类型', name: 'organizationType' , width: 100 },
                { display: '机构类别', name: 'organizationCategory' , width: 100 },
                { display: '学科分类', name: 'sortSubject' , width: 100},
                { display: '组成形式', name: 'modusComposition' , width: 100},
                { display: '从业人员（合计）', name: 'totalEmployees' , width: 150},
                { display: '科技活动人员【人年】（合计）', name: 'totalIts' , width: 170},
                { display: '培养研究生（人）', name: 'numGraduates' , width: 150},
                { display: '当年经费内部支出（千元）', name: 'internalExpenditures' , width: 200},
                { display: '承担课题（项）', name: 'numIssueAssume' , width: 100},
                { display: '固定资产原值（千元）', name: 'assetsFixed' , width: 150},
                { display: '服务的国民经济行业', name: 'industryService' , width: 150},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewScienceOrganizationDetail.action?flag=modify&infoid='+row.organizationId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.organizationName);
                                }else
                                {
                                	checkedIds.push(this.organizationId);
                                	checkedNames.push(this.organizationName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteScienceOrganization.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
