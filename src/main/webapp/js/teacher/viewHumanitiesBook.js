 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '成果名称', name: 'bookName', align: 'left', minWidth: 200 },
                { display: '等级', name: 'grades', width: 100 },
                { display: '出版单位', name: 'publisher', width: 150 },
                { display: '出版日期', name: 'publishedTime' , width: 150},
                { display: '书号（ISBN）', name: 'iSBN' , width: 150},
                { display: '字数（千字）', name: 'wordcount' , width: 100},
                { display: '成果所属项目', name: 'belongProject' , width: 150},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewHumanitiesBookDetail.action?flag=modify&infoid='+row.bookId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.bookName);
                                }else
                                {
                                	checkedIds.push(this.bookId);
                                	checkedNames.push(this.bookName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteHumanitiesBook.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
