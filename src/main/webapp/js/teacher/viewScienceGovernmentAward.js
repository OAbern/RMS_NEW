 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                ...
                { display: '主要完成单位', name: 'completeUnit' , width: 200},
                { display: '配套单位奖励金额（万元）', name: 'unitAward' , width: 200},
                { display: '配套个人奖励金额（万元）', name: 'personAward' , width: 200},
                { display: '合计奖励金额（万元）', name: 'totalAward' , width: 200},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewScienceGovernmentAwardDetail.action?flag=modify&infoid='+row.awardId+'">修改</a>';
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteScienceGovernmentAward.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
