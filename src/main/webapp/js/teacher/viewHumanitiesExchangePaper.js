 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '论文名称', name: 'exchangePaperName', align: 'left', minWidth: 250 },
                { display: '检索情况', name: 'searchStation', width: 200 },
                { display: '学科分类', name: 'subjectsClassify', width: 100 },
                { display: '发表时间', name: 'publishedTime' , width: 150},
                { display: '主办会议名称', name: 'hostConferenceName' , width: 200},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewHumanitiesExchangePaperDetail.action?flag=modify&infoid='+row.exchangePaperId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.exchangePaperName);
                                }else
                                {
                                	checkedIds.push(this.exchangePaperId);
                                	checkedNames.push(this.exchangePaperName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteHumanitiesExchangePaper.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
