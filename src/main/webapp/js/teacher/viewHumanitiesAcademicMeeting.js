 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '会议名称', name: 'academicMeetingName', align: 'left', minWidth: 250 },
                { display: '主办单位', name: 'hostUnit', width: 200 },
                { display: '会议类型', name: 'meetingClassify', width: 150 },
                { display: '举办时间', name: 'holdingTime' , width: 100},
                { display: '会议地点', name: 'meetingLocation' , width: 200},
                { display: '参加人数', name: 'participantsNumber' , width: 100},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewHumanitiesAcademicMeetingDetail.action?flag=modify&infoid='+row.academicMeetingId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.academicMeetingName);
                                }else
                                {
                                	checkedIds.push(this.academicMeetingId);
                                	checkedNames.push(this.academicMeetingName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteHumanitiesAcademicMeeting.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
