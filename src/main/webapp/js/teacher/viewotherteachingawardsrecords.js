                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteOtherTeachingAwardsRecord.action?recordIdString='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
                             ...
                             if(checkedStatus == "审批通过"||checkedStatus == "未审批"){
                            ...
                            else if(checkedIds.length == data.length)
                               {
	                            	...
	                            	$.ligerDialog.confirm('确定修改:' + checkedNames.join(' ; ') + '?', function (result)
	                                {
	                            		if(result)
	                            		{
	                                       		url = 'viewOtherTeachingAwardsRecordDetail.action?flag=modify'+'&recordId='+checkedIds;
	                                       		modifyInfo(url);
	                                    }
	                            	 });
                             }
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定查看:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'viewOtherTeachingAwardsRecordDetail.action?recordId='+checkedIds;
                                    		detailInfo(url);
                                        }
                                });
                             }
