        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                         ...
                         { display: '排名第一的学生姓名', name: 'firstStudents', align: 'left', width: 100, minWidth: 120 },
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewStudentAwardsNewDetail.action?flag=modify&infoid='+row.awardsId+'">修改</a>';
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteStudentAwardsNew.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
