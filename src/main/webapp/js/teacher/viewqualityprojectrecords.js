                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteQualityProjectRecord.action?recordIdString='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
                             ...
                             if(checkedStatus == "审批通过"||checkedStatus == "未审批"){
                            ...
                            else if(checkedIds.length == data.length)
                               {
	                            	...
	                            	$.ligerDialog.confirm('确定修改:' + checkedNames.join(' ; ') + '?', function (result)
	                                {
	                            		if(result)
	                            		{
	                                       		url = 'viewQualityProjectRecordDetail.action?flag=modify'+'&recordId='+checkedIds;
	                                       		modifyInfo(url);
	                                    }
	                            	 });
                             }
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定查看:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'viewQualityProjectRecordDetail.action?recordId='+checkedIds;
                                    		detailInfo(url);
                                        }
                                });
                             }
                    ...
                    case "Excel":
                        ...
                        else
                        {
                        	...
                        	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                        	{
                        		if(result)
                        		{
                        			link = 'downLoadExcelbyFactor/downloadDynamicDataRecordExcel.action?factorName=id&factorValue='+checkedIds+'&classNum=6';
                        			downloadExcel(link);
                        		}
                        	});
                        }
