 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '级别', name: 'classAchievements', align: 'left', minWidth: 100 },
                { display: '项目名称', name: 'projectName', width: 250 },
                { display: '获奖等级', name: 'levelAchievements', width: 100 },
                { display: '时间', name: 'timeAchievements', width: 150 },
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewTeachAchievementsDetail.action?flag=modify&infoid='+row.achievementsId+'">修改</a>';
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteTeachAchievements.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
