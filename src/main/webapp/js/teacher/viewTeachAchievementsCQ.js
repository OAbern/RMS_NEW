...
var manager;
 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            //工具条
            $("#toptooslbar").ligerToolBar({ items: [
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewTeachAchievementsCQDetail.action?flag=modify&infoid='+row.achievementsId+'">修改</a>';
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteTeachAchievementsCQ.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
