 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                ...
                { display: '专业名称', name: 'majorName', width: 200 },
                { display: '年度检查时间', name: 'checkTime', width: 150 },
                { display: '结题时间', name: 'endTime', width: 150 },
                { display: '学院奖励', name: 'rewardCollege' , width: 150},
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewMajorContributeDetail.action?flag=modify&infoid='+row.majorId+'">修改</a>';
                    ...
                    case "delete":
                        ...
                        else
                        {
                            ...
                            $(data).each(function ()
                            {
                                ...
                                {
                                	alert('不能删除:'+this.majorName);
                                }else
                                {
                                	checkedIds.push(this.majorId);
                                	checkedNames.push(this.majorName);
                                }
                            });
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteMajorContribute.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
