	...
	$.extend($.XYTipsWindow,{
		...
		contentBox: function (o) {
			...
			switch($contentType) {
				...
				$.ajax({
					...
					success:function(html){
						$contentID.html("<iframe id='XYTipsWindows_iframe' src=\""+$content+"\" width=\"100%\" height=\""+parseInt(o.___height)+"px\" scrolling=\"auto\" frameborder=\"0\" marginheight=\"0\" marginwidth=\"0\"></iframe>");
					}
				});
			};
		},
		...
		removeBox: function (){
			...
			if($box != null || $boxbg != null){
				if(BOXID.___flush==true){
				   var popuplayers=$(".popupLayer");
				   for(var zi=0;zi<popuplayers.length;zi++){
					   if(popuplayers[zi].style.display=="block"){
						    popuplayers[zi].getElementsByTagName("iframe")[0].contentWindow.load();
					   }
				   }
				}
				var $contentID = $(".___boxContent",$box);
