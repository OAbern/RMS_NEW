 ...
 var alert = function (content)
        ...
        function showdata(){
            ...
            g = manager = $grid = $("#maingrid").ligerGrid({
                ...
                { display: '课程名称', name: 'courseName', align: 'left', width: 160, minWidth: 60 },
                ...
                { display: '审批者', name: 'approvedUser', align: 'left', width: 80, minWidth: 60 , frozen: true},
                   { display: '操作', isAllowHide: false, width: 80, frozen: true,
                   	render: function (row)
                       {
                   		var html = '<a href="viewCourseContributeDetail.action?infoid='+row.courseId+'">查看详细</a>';
                           return html;
                       }
                   }
        ...
        function getInfo(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"searchCollegeCourseContribute.action",
        		datatype:"script",
        ...
        function callback(builder){
        	...
        	for(var objid in callbackdate){
        		...
        		var row = {courseId: courseContributeInfo.courseId,
        				classContribute: courseContributeInfo.classContribute,
        				typeContribute: courseContributeInfo.typeContribute, 
        				timeContribute:courseContributeInfo.timeContribute, 
        				courseName:courseContributeInfo.courseName,
        				checkTime:courseContributeInfo.checkTime,
        				endTime:courseContributeInfo.endTime,
        				collegeAward:courseContributeInfo.collegeAward,
        				submitUser:courseContributeInfo.submitUser,
        				approvedUser:courseContributeInfo.approvedUser,
        				Status: courseContributeInfo.Status
        				};
        		rows.push(row);
        	}
        	showdata();	
                	...
                	case "Excel":
                        ...
                        else
                        {
                        	...
                        	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                        	{
                        		if(result)
                        		{
                        			link = 'downLoadExcelbyFactor/downloadCourseContributeExcel.action?factorName=courseId&factorValue='+checkedIds;
                        			downloadExcel(link);
                        		}
                        	});
                        }
