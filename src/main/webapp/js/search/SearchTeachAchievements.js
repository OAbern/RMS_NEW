 ...
 var alert = function (content)
        ...
        function showdata(){
            ...
            g = manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '成果级别', name: 'classAchievements', align: 'left', width: 100, minWidth: 60 },
                { display: '项目名称', name: 'projectName', align: 'left', width: 100, minWidth: 60 },
                { display: '等级', name: 'levelAchievements', align: 'left', width: 100, minWidth: 60 },
                { display: '时间', name: 'timeAchievements', align: 'left', width: 100, minWidth: 60 },
                { display: '学院奖励', name: 'collegeAward', align: 'left', width: 100, minWidth: 60 },
                ...
                { display: '操作', isAllowHide: false, width: 80, frozen: true,
                   	render: function (row)
                       {
                   		var html = '<a href="viewTeachAchievementsDetail.action?infoid='+row.achievementsId+'">查看详细</a>';
                           return html;
                       }
                   }
        ...
        function getInfo(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"searchCollegeTeachAchievementsInfo.action",
        		datatype:"script",
        ...
        function callback(builder){
        	...
        	for(var objid in callbackdate){
        		var teachAchievementsInfo = callbackdate[objid];		
        		var row = {achievementsId: teachAchievementsInfo.achievementsId,
        				classAchievements: teachAchievementsInfo.classAchievements,
        				projectName: teachAchievementsInfo.projectName, 
        				levelAchievements:teachAchievementsInfo.levelAchievements, 
        				timeAchievements:teachAchievementsInfo.timeAchievements,
        				collegeAward:teachAchievementsInfo.collegeAward,
        				submitUser:teachAchievementsInfo.submitUser,
        				approvedUser:teachAchievementsInfo.approvedUser,
        				Status: teachAchievementsInfo.Status
        				};
        		rows.push(row);
        	}
        	showdata();	
                ...
                case "Excel":
                    ...
                    else
                    {
                    	...
                    	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                    	{
                    		if(result)
                    		{
                    			link = 'downLoadExcelbyFactor/downloadTeachAchievementsExcel.action?factorName=achievementsId&factorValue='+checkedIds;
                    			downloadExcel(link);
                    		}
                    	});
                    }
