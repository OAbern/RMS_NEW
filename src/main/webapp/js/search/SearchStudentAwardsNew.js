...
var manager, g;
 ...
 var alert = function (content)
        ...
        function showdata(){
            ...
            g = manager = $grid = $("#maingrid").ligerGrid({
                ...
                { display: '操作', isAllowHide: false, width: 80, frozen: true,
                   	render: function (row)
                       {
                   		var html = '<a href="viewStudentAwardsNewDetail.action?infoid='+row.awardsId+'">查看详细</a>';
                           return html;
                       }
                   }
        ...
        function getInfo(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"searchCollegeStudentAwardsNewInfo.action",
        		datatype:"script",
        ...
        function callback(builder){
        	...
        	for(var objid in callbackdate){
        		var studentAwardsNewInfo = callbackdate[objid];		
        		var row = {awardsId: studentAwardsNewInfo.awardsId,
        				rewardTime: studentAwardsNewInfo.rewardTime,
        				rewardStudents: studentAwardsNewInfo.rewardStudents, 
        				rewardName:studentAwardsNewInfo.rewardName, 
        				rewardLevel:studentAwardsNewInfo.rewardLevel,
        				collegeAward:studentAwardsNewInfo.collegeAward,
        				submitUser:studentAwardsNewInfo.submitUser,
        				approvedUser:studentAwardsNewInfo.approvedUser,
        				Status: studentAwardsNewInfo.Status,
        				firstStudents:studentAwardsNewInfo.firstStudents,
        				remarks:studentAwardsNewInfo.remarks
        				};
        		rows.push(row);
        	}
        	showdata();	
                ...
                case "Excel":
                    ...
                    else
                    {
                    	...
                    	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                    	{
                    		if(result)
                    		{
                    			link = 'downLoadExcelbyFactor/downloadStudentAwardsNewExcel.action?factorName=awardsId&factorValue='+checkedIds;
                    			downloadExcel(link);
                    		}
                    	});
                    }
