 ...
 var alert = function (content)
        ...
        function showdata(){
            ...
            g = manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '交流论文名称', name: 'exchangePaperName', align: 'left', width: 160, minWidth: 60 },
                ...
                { display: '发表时间', name: 'publishedTime', align: 'left', width: 100, minWidth: 60 },
                { display: '主办会议名称', name: 'hostConferenceName', align: 'left', width: 100, minWidth: 60 },
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                   	render: function (row)
                       {
                   		var html = '<a href="viewHumanitiesExchangePaperDetail.action?infoid='+row.exchangePaperId+'">查看详细</a>';
                           return html;
                       }
                   }
                ], dataAction: 'server', data: rows, sortName: 'exchangePaperId',
       ...
       function getInfo(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"searchCollegeHumanitiesExchangePaper.action",
        		datatype:"script",
        ...
        function callback(builder){
        	...
        	for(var objid in callbackdate){
        		var humanitiesExchangePaperInfo = callbackdate[objid];		
        		var row = {exchangePaperId: humanitiesExchangePaperInfo.exchangePaperId,
        				exchangePaperName: humanitiesExchangePaperInfo.exchangePaperName,
        				searchStation: humanitiesExchangePaperInfo.searchStation, 
        				subjectsClassify:humanitiesExchangePaperInfo.subjectsClassify, 
        				publishedTime:humanitiesExchangePaperInfo.publishedTime,
        				hostConferenceName:humanitiesExchangePaperInfo.hostConferenceName,
        				submitUser:humanitiesExchangePaperInfo.submitUser,
        				approvedUser:humanitiesExchangePaperInfo.approvedUser,
        				Status: humanitiesExchangePaperInfo.Status
        				};
        		rows.push(row);
        	}
        	showdata();	
                ...
                switch (item.id)
                {
                	...
                	case "Excel":
                        ...
                        else
                        {
                        	...
                        	$(data).each(function ()
                            {
                        		checkedIds.push(this.exchangePaperId);
                            });
                        	$.ligerDialog.confirm('确定导出数据已选择的数据?', function (result)
                        	{
                        		if(result)
                        		{
                        			link = 'downLoadExcelbyFactor/downloadHumanitiesExchangePaperExcel.action?factorName=exchangePaperId&factorValue='+checkedIds;
                        			downloadExcel(link);
                        		}
                        	});
                        }
