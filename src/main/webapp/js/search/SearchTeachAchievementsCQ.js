 ...
 var alert = function (content)
        ...
        function showdata(){
            ...
            g = manager = $grid = $("#maingrid").ligerGrid({
                columns: [
				{ display: '项目编号', name: 'reportedAmounts', align: 'left', width: 100, minWidth: 60 },
				{ display: '项目名称', name: 'projectName', align: 'left', minWidth: 140 },
				{ display: '立项时间', name: 'timeAchievements', align: 'left', minWidth: 140 },
				{ display: '结题时间', name: 'gradeAchievements', align: 'left', minWidth: 140 },
				{ display: '类型', name: 'projectType', align: 'left', minWidth: 140 },
				{ display: '负责人（学生）', name: 'classAchievements', align: 'left', minWidth: 140 },
				{ display: '团队成员', name: 'certificationUnit', align: 'left', minWidth: 140 },
				{ display: '备注', name: 'remarks', align: 'left', minWidth: 140 },
				{ display: '奖励金额', name: 'verifyAmounts', align: 'left', minWidth: 140 },
                ...
                { display: '操作', isAllowHide: false, width: 80, frozen: true,
                   	render: function (row)
                       {
                   		var html = '<a href="viewTeachAchievementsCQDetail.action?infoid='+row.achievementsId+'">查看详细</a>';
                           return html;
                       }
                   }
        ...
        function getInfo(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"searchCollegeTeachAchievementsCQ.action",
        		datatype:"script",
        ...
        function callback(builder){
        	...
        	for(var objid in callbackdate){
        		var teachAchievementsCQInfo = callbackdate[objid];		
        		var row = {achievementsId: teachAchievementsCQInfo.achievementsId,
        				projectName: teachAchievementsCQInfo.projectName, 
        				timeAchievements:teachAchievementsCQInfo.timeAchievements,
        				submitUser:teachAchievementsCQInfo.submitUser,
        				approvedUser:teachAchievementsCQInfo.approvedUser,
        				Status: teachAchievementsCQInfo.Status,
        				reportedAmounts: teachAchievementsCQInfo.reportedAmounts,
        				gradeAchievements:teachAchievementsCQInfo.gradeAchievements,
        				projectType:teachAchievementsCQInfo.projectType,
        				certificationUnit:teachAchievementsCQInfo.certificationUnit,
        				remarks:teachAchievementsCQInfo.remarks,
        				verifyAmounts:teachAchievementsCQInfo.verifyAmounts
                ...
                case "Excel":
                    ...
                    else
                    {
                    	...
                    	$.ligerDialog.confirm('确定导出已选择的数据?', function (result)
                    	{
                    		if(result)
                    		{
                    			link = 'downLoadExcelbyFactor/downloadTeachAchievementsCQExcel.action?factorName=achievementsId&factorValue='+checkedIds;
                    			downloadExcel(link);
                    		}
                    	});
                    }
