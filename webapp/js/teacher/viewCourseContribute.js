 ...
 var alert = function (content)
        ...
        $(function ()
        {
            ...
            manager = $grid = $("#maingrid").ligerGrid({
                columns: [
                { display: '立项级别', name: 'classContribute', align: 'left', minWidth: 100 },
                { display: '立项类别', name: 'typeContribute', width: 100 },
                { display: '立项时间', name: 'timeContribute', width: 150 },
                { display: '课程名称', name: 'courseName', width: 200 },
                { display: '年度检查时间', name: 'checkTime', width: 150 },
                { display: '结题时间', name: 'endTime', width: 150 },
                ...
                { display: '操作', isAllowHide: false, width: 60, frozen: true,
                	render: function (row)
                    {
                		var html = '<a href="viewCourseContributeDetail.action?flag=modify&infoid='+row.courseId+'">修改</a>';
                        ...
                        else
                        {
                            ...
                            if(checkedIds.length == data.length)
                            {
                            	$.ligerDialog.confirm('确定删除:' + checkedNames.join(' ; ') + '?', function (result)
                                {
                                    if(result)
                                    	{
                                    		url = 'deleteCourseContribute.action?infoIds='+checkedIds;
                                    		deleteInfo(url);
                                        }
                                });
                            }
